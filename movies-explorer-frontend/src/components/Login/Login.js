	...
	constructor(props){
			...
			this.handleSubmit = this.handleSubmit.bind(this);
				...
				handlePasswordChange =(e) => {
					const validPassword =  /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\s).*$/i.test(e.target.value);

					if (!validPassword || e.target.value.length < 10) {
						this.setState({passwordErr: "Пароль должен содержать не менее 10 символов и состоять из букв и цифрр"})
				} else {
