  ...
  <section className="profile">
			 <Header color='black'/>
				<div className="profile__info-block">
					 {nameIsChanged === false &&(<p className="profile__greeting">{`Привет, ${currentUser.data.name}!`}</p>)}
						{nameIsChanged === true &&(<p className="profile__greeting">{`Привет, ${currentUser.name}!`}</p>)}
						<form className="profile__form" onSubmit={handleSubmit}>
							<div className="profile__form-item">
							 <span className="profile__input-title">Имя</span>
							 {editingMode === false && (<input className="profile__input" type="text" name="name" value={currentUser.name || "" } placeholder={currentUser.data.name} onChange={handleNameChange} disabled/>)}
								{editingMode === true && (<input className="profile__input" type="text" name="name" value={name} placeholder={currentUser.data.name} onChange={handleNameChange} />)}
								</div>
								<p className="profile__input-err">{nameError}</p>
								<div className="profile__form-item">
								<span className="profile__input-title">Email</span>
							 {editingMode === false && (<input className="profile__input" type="text" name="email" value={currentUser.email || "" } placeholder={currentUser.data.email} onChange={handleEmailChange} disabled/>)}
								{editingMode === true && (<input className="profile__input" type="text" name="email" value={email} placeholder={currentUser.data.email} onChange={handleEmailChange}/>)}
								</div>
